  ...
  return (
    <div className="session-editor-backdrop" onClick={onClose} style={{ zIndex: 10001 }}>
      <div className="session-editor" onClick={e => e.stopPropagation()} style={{ width: 600, height: 520, display: 'flex', flexDirection: 'column' }}>
        <h3 style={{ marginBottom: 8 }}>{mode === 'folder' ? '📁 디렉토리 선택' : '📄 파일 선택'} <span style={{ fontSize: 12, color: '#888', fontWeight: 'normal' }}>{sourceLabel}</span></h3>

        {/* 경로 바 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          <button onClick={goUp} title="상위" style={{ padding: '3px 8px' }}>▲</button>
          ...
          <button onClick={() => navigate(editPath)} title="이동" style={{ padding: '3px 8px' }}>↵</button>
          <button onClick={() => load(path)} title="새로고침" style={{ padding: '3px 8px' }}>⟳</button>
        </div>
        ...
        <div style={{ flex: 1, minHeight: 0, border: '1px solid #333', borderRadius: 4, background: '#161616', overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ color: '#666', fontSize: 12, padding: 16, textAlign: 'center' }}>(비어있음)</div>
          ) : (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          <span style={{ fontSize: 11, color: '#888' }}>
            {mode === 'folder' ? `현재 디렉토리를 선택합니다: ${path}` : (selectedFile ? `파일: ${selectedFile}` : '파일을 선택하세요')}
          </span>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onClose}>취소</button>
            <button className="primary" onClick={confirmPick} disabled={mode === 'file' && !selectedFile}>선택</button>
          </div>
        </div>
      </div>
    </div>
